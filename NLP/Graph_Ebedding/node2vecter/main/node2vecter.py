...
def get_engine():
    ...
    engine = 'mysql+pymysql://root:123456@172.30.6.205:3306/jeecg-boot?charset=utf8'
    return engine

class node2vecter(base):
    def process(self):
        connection = pymysql.connect(host='172.30.6.204', user='root', password='123456', database='jeecg-boot',
                                     port=int(3306))
        try:
            ...
            connection.close()
        x = graph_labels['f1'].tolist()
        y = graph_labels['f2'].tolist()

        #G = nx.from_pandas_edgelist(graph_feature,
        #                     create_using=nx.DiGraph(), nodetype=None, data=[('weight', int)])
        G = nx.from_pandas_edgelist(graph_feature, create_using=nx.Graph())
