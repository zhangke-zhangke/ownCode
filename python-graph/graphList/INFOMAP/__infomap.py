...
class Infomap:
    ...
    def findCommunities(self, G):
        ...
        """
        # 节点id最大使用c++ 32bit long ,range 0 to 4294967295
        # 改进使用
        infomapWrapper = infomap.Infomap("--two-level --directed",silent=True)
        self.mapping = infomapWrapper.add_networkx_graph(G,weight='weight')
        print(self.mapping)
        ...
        for node in infomapWrapper.iterLeafNodes():
            communities[self.mapping.get(node.physicalId)] = node.moduleIndex()
        # 上面为改进版本
        # for node in infomapWrapper.iterLeafNodes():
    ...
    def printCom(self, G):
        ...
        for communitie in communitie_sort:
            # 增加打印输出大于等于2的社区
            if communitie.__len__() >= 2:
                count += 1
                print(f'社区{count},成员人数{communitie.__len__()},成员{communitie}', end='\n')
        print(self.cal_Q(communities.values()))

    def get_communities_modules(self,node):
        # 通过点找社区id
        # self.mapping : {0: '111111111111111', 1: '222222222222222'}
    ...
    def drawNetwork(self, G):
        ...
        for n in G.nodes():
            plt.annotate(n,
                         ...
                         xytext=[0, 0],
                         color=cmapDark(communities[self.get_communities_modules(n) - 1])
                         )
...
# # graph = obj.createGraph("data/OpenFlights_test.txt")
# graph = G
# print(graph)
#
# # import sys
