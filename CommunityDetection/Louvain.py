...
if __name__ == '__main__':
    ...
    #     and 交易金额 > 10
    #     and 交易对方名称 not regexp '支付宝|财付通|支付|微粒贷|信贷|特约|花呗|借呗'
    #     limit 1000
    # '''
    # sqlDf = getSqlResult(sql)
    #
    # print(sqlDf.columns)
    #
    # G = nx.from_pandas_edgelist(sqlDf, source='查询卡号', target='交易对方卡号', edge_attr=['交易金额'],
    ...
    G = load_graph('data/OpenFlights_test.txt')
    # G = load_graph('data/dummy.txt')
    obj = Graph()
    G1 = obj.createGraph("data/OpenFlights_test.txt")
    ...
    for communitie in communities:
        count += 1
        print("社区", count,f',共{communitie.__len__()}人 ', communitie)
